...
int main()
{
    ...
    {
	    snprintf(infile, sizeof(infile), "D:/efg/bad apple/bmp/%d.bmp", cnt_file);
	    snprintf(bin_file_name, sizeof(bin_file_name), "D:/efg/bad apple/bin/%d.bin", cnt_file);
	    ...
	    for (i = 0; i < height/8; i++)
	    {
	        for (j = 0; j < width; j++)
	        {
	            for (k = 0; k < 8; k++)
	            {
	                if (bin_buff[j + k*width + i*width*8])
	                    byte_buff[j + i*width] += (1 << k);
	            }
	        }
	    }
