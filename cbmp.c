...
BMP* bopen(char* file_path)
{
    ...
    bmp->depth = 24;//_get_depth(bmp->file_byte_contents);

    if(!_validate_depth(bmp->depth))
    {
        printf("Invalid file depth (%u)\n", bmp->depth);
...
int main()
{
    ...
    char name_buf[10];
    unsigned int cnt_file = 0, name_size;
    unsigned int i, j, k, x, y, width, height, luminance;
    unsigned char r, g, b;

    snprintf(bin_file_name, sizeof(bin_file_name), "D:/your_path/%d.bin", 777);
    outfile = fopen(bin_file_name, "wb");
            
    for(cnt_file = 0; cnt_file <= 6572; cnt_file++)
    {
	    snprintf(infile, sizeof(infile), "D:/your_path/%d.bmp", cnt_file);
	    


	    printf("Input name: %s --> ", infile);
	    ...
	    uint8_t bin_buff[width * height],
	            byte_buff[width * height/8];
	    ...
	    for (y = 0; y < height; y++)
	    {
	        for (x = 0; x < width; x++)
	        {
	            ...
	            luminance = (unsigned int)(r*0.299 + g*0.587 + b*0.114);
	            if (luminance >= 150)
	            {
	                bin_buff[i] = 0;
	    ...
	    for (y = 0; y < height/8; y++)
	    {
	        for (x = 0; x < width; x++)
	        {
	            for (k = 0; k < 8; k++)
	            {
	                if (bin_buff[x + k*width + y*width*8])
	                    byte_buff[x + y*width] += (1 << k);
	            }
	        }
	    }
        fseek(outfile, cnt_file*1024, SEEK_SET);
	    fwrite(byte_buff, sizeof(uint8_t), width * height/8, outfile);
...
/*
    ...
    snprintf(name_buf, sizeof(name_buf), "%d", cnt_file);
    name_size = strlen(name_buf) + 4;
    printf("name_buf=%d\n", name_size);
    snprintf(infile, name_size, "%s.bmp", name_buf);
    snprintf(bin_file_name, name_size, "%s.bin", name_buf);
*/
